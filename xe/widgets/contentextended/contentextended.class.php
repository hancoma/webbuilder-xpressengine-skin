<?php
    ...
    class contentextended extends WidgetHandler {
        ...
        function proc($args) {
            ...
            // rss가 아닌 XE모듈일 경우 모듈 번호 정리 후 모듈 정보 구함
                ...
                if(!$args->module_srls){
                    ...
                    $output = executeQueryArray('widgets.contentextended.getMids', $obj);
                    if($output->data) {
        ...
        /**
         * @brief 댓글 목록을 추출하여 contentextendedItem으로 return
         **/
        function _getCommentItems($args) {
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.contentextended.getNewestCommentList', $obj);
				// 더이상 결과가 없으면 break
            ...
            foreach($result as $key => $oComment) {
                ...
                $content_item = new contentextendedItem($browser_title);
				...
				if ($args->show_content_title == 'Y') {
					...
					$docu_output = executeQuery('widgets.contentextended.getDocument', $docu_args);
					if ($docu_output->data) {
        ...
        function _getDocumentItems($args){
            ...
            if($getcount > 0) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new contentextendedItem( $args->module_srls_info[$module_srl]->browser_title );
        ...
        function _getRssItems($args){
            ...
            if($xml_doc->rss) {
                ...
                foreach ($items as $key => $value) {
                    ...
                    $content_item = new contentextendedItem($rss->title);
    ...
    class contentextendedItem extends Object {
