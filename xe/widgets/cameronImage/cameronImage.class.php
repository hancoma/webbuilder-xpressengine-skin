<?php
    ...
    class cameronImage extends WidgetHandler {
        function proc($args) {
            // 정렬 대상
            if(!in_array($args->order_target, array('list_order','update_order','regdate','voted_count','readed_count','rand()'))) $args->order_target = 'list_order';
            ...
            // 썸네일 가로 크기
            if(!$args->thumbnail_width) $args->thumbnail_width = 320;

            // 썸네일 세로 크기
            if(!$args->thumbnail_height) $args->thumbnail_height = 320;
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.cameronImage.getMids', $obj);
				if($output->data) {
        ...
        function _getDocumentItems($args){
            ...
            $output = executeQueryArray('widgets.cameronImage.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.cameronImage.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
            ...
            if($getcount > 0) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new cameronImageItem( $args->module_srls_info[$module_srl]->browser_title );
    ...
    class cameronImageItem extends Object {
        ...
        function cameronImageItem($browser_title=''){
