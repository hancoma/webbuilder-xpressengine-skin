<?php
    /**
     * @class cameronWebzine
     * @author CAMERON (mail@cameron.co.kr)
     **/

    class cameronWebzine extends WidgetHandler {
        function proc($args) {
            // 정렬 대상
            if(!in_array($args->order_target, array('list_order','update_order','regdate','rand()','voted_count'))) $args->order_target = 'list_order';
            ...
            // 썸네일 가로 크기
            if(!$args->thumbnail_width) $args->thumbnail_width = 60;
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.cameronWebzine.getMids', $obj);
				if($output->data) {
        ...
        function _getDocumentItems($args){
            ...
            $output = executeQueryArray('widgets.cameronWebzine.getCategories',$obj);
			...
			$pagecount = 0;
			$needcount = $args->list_count * $args->page_count; // 가져와야 하는 글 개수
			$getcount = 0; // 가져온 글 개수
            $content_items = array();
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.cameronWebzine.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
            ...
            if($getcount > 0) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new cameronWebzineItem( $args->module_srls_info[$module_srl]->browser_title );
    ...
    class cameronWebzineItem extends Object {
        ...
        function cameronWebzineItem($browser_title=''){
