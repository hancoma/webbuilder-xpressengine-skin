<?php
    /**
     * @class cameronListOne
     * @author CAMERON (mail@cameron.co.kr)
     **/

    class cameronListOne extends WidgetHandler {
        function proc($args) {
            // 정렬 대상
            if(!in_array($args->order_target, array('list_order','update_order','regdate','voted_count','rand()'))) $args->order_target = 'list_order';
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.cameronListOne.getMids', $obj);
				if($output->data) {
        ...
        function _getDocumentItems($args){
            ...
            $output = executeQueryArray('widgets.cameronListOne.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.cameronListOne.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
            ...
            if($getcount > 0) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new cameronListOneItem( $args->module_srls_info[$module_srl]->browser_title );
    ...
    class cameronListOneItem extends Object {
        ...
        function cameronListOneItem($browser_title=''){
