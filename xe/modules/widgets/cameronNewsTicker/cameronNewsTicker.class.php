<?php
    /**
     * @class cameronNewsTicker
     * @author CAMERON (mail@cameron.co.kr)
     **/

    class cameronNewsTicker extends WidgetHandler {
        function proc($args) {
            // 정렬 대상
			...
			if(!$args->module_srls){
				...
				$output = executeQueryArray('widgets.cameronNewsTicker.getMids', $obj);
				if($output->data) {
        ...
        function _getDocumentItems($args){
            ...
            $output = executeQueryArray('widgets.cameronNewsTicker.getCategories',$obj);
			...
			while(true) {
				...
				$obj->page = $pagecount;
				$output = executeQueryArray('widgets.cameronNewsTicker.getNewestDocuments', $obj);
				// 더이상 결과가 없으면 break
            ...
            if($getcount > 0) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new cameronNewsTickerItem( $args->module_srls_info[$module_srl]->browser_title );
    ...
    class cameronNewsTickerItem extends Object {
        ...
        function cameronNewsTickerItem($browser_title=''){
